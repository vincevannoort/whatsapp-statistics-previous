var app = new Vue({
  ...
  computed: {
    done_analysing: function () { return this.messages.length != 0; },
    statistics_messages_count: function() { return this.messages.length; },
    statistics_person_messages_count: function() {
      var counted_messages = this.messages.reduce(function (all_persons, line) {
        if (line.person in all_persons) { all_persons[line.person]++;} 
        else { all_persons[line.person] = 1; }
        return all_persons;
      }, {});
      ...
      var counted_emojis = this.messages.reduce(function (all_emojis, line) {
        ...
        if (emoji_array != null) {
          emoji_array.forEach(function(emoji){
            if (emoji in all_emojis) { all_emojis[emoji]++;} 
            else { all_emojis[emoji] = 1;}
          });
        }
        return all_emojis;
      }, {});
    ...
    statistics_messages_per_day_count: function() {
      var counted_messages_by_day = this.messages.reduce(function (all_dates, line) {
        if (line.date in all_dates) { all_dates[line.date]++; } 
        else { all_dates[line.date] = 1; }
        return all_dates;
      }, {});
    ...
    statistics_messages_per_month_count: function() {
      var counted_messages_by_month = this.messages.reduce(function (all_dates, line) {
        if (line.date.substr(3,5) in all_dates) { all_dates[line.date.substr(3,5)]++; } 
        else { all_dates[line.date.substr(3,5)] = 1; }
        return all_dates;
      }, {});
    ...
    statistics_messages_per_hour: function() {
      var counted_messages_by_hour = this.messages.reduce(function (all_times, line) {
        if (parseInt(line.time.substr(0,2)) in all_times) { all_times[parseInt(line.time.substr(0,2))]++; } 
        else { all_times[parseInt(line.time.substr(0,2))] = 1; }
        return all_times;
      }, {});
  ...
  watch: {
    statistics_messages_count: 'setup_charts',
    statistics_person_messages_count: 'setup_charts',
    statistics_emoji_count: 'setup_charts',
    statistics_messages_per_day_count: 'setup_charts',
  },
  methods: {
    ...
    setup_time_between_messages: function(){
      var self = this;
      this.messages.forEach(function (line, index) {
        if (index != self.messages.length-1) {
          ...
          var next_message = moment(self.messages[index+1].time, "HH:mm:ss");
          var difference_between = moment.utc(next_message.diff(first_message));
          self.messages[index].time_till_next_messages = difference_between;
        }
      });
    },
    setup_charts: _.debounce(function() {
      this.setup_time_between_messages();
